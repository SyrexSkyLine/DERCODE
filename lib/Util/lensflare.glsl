...
void LensFlare(inout vec3 color, vec2 lightPos, float truePos, float multiplier) {
    ...
    if (falloffOut < 0.999) {
        vec3 lensFlare = (
            ...
            BaseLens(lightPos, 0.3,  1.15, 1.0) * vec3(0.08, 0.1, 2.8) * 0.12 +
            BaseLens(lightPos, 0.3,  1.35, 1.0) * vec3(0.04, 0.1, 3.4) * 0.14 +
