...
layout(location = 3) out vec3 emissiveOut; // <-- новый эмиссивный выход
...
// Параметры для bloom/молнии
uniform float bloomThreshold;   // порог, выше которого пиксель считается источником свечения (напр., 1.0)
uniform float bloomStrength;    // множитель для выходного эмиссива (напр., 1.5..4.0)
uniform float frameTimeCounter; // если хочешь пульсацию при ударе
...
void main() {
    ...
    // ---- вычисляем эмиссив для bloom ----
    // Если пиксель ярче порога, берём лишек как источник свечения.
    // Для явной молнии ты можешь установить albedo в другом месте как vec3(3.0).
    vec3 excess = max(albedo.rgb - vec3(bloomThreshold), vec3(0.0));
    vec3 emissive = excess * bloomStrength;

    // небольшой дополнительный импульс для тех пикселей, у которых materialIDs == 101 (если ты рендеришь молнию этим ID)
    if (materialIDs == 101) {
        float pulse = 1.0 + sin(frameTimeCounter * 60.0) * 0.25; // быстрая небольшая пульсация
        emissive *= pulse;
    }
