...
void main() {
	...
	if (depth < 1.0) {
		...
		if (materialMaskT.translucent) {
			float rDist = length(ScreenToViewSpace(vec3(refractCoord, GetDepthSoild(refractTexel))));
	...
	if (!materialMaskT.water && !materialMaskT.stainedGlass && !materialMaskT.ice) NetherFog(sceneData, fogDist);
	...
	CommonFog(sceneData, fogDist);
		...
		fogTransmittance = expf(-fogDensity * fogDist);
	#endif
